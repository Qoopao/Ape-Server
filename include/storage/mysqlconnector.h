...
public:
  ...
  // 返回一个指定的executor，用于数据库事务
  boost::mysql::connection_pool &get_pool(int idx) {
    return *_pools[idx % _poolSize];
  }

  boost::asio::io_context::executor_type get_pool_executor(int idx) {
    return _iocPool->get_ioc(idx % _poolSize).get_executor();
  }
  ...
  async_execute(const Stmt &stmt) {
    ...
    co_await boost::asio::dispatch(boost::asio::bind_executor(
        ioc.get_executor(), boost::asio::use_awaitable));
