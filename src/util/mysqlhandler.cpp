...
sdkws::GroupInfo parse_group_from_row(const boost::mysql::rows_view &rows)
{
    auto row = rows.at(0);
    sdkws::GroupInfo g;
    g.set_groupid(row.at(0).as_string());
    g.set_groupname(row.at(1).as_string());
    if (!row.at(2).is_null()) g.set_notification(row.at(2).as_string());
    if (!row.at(3).is_null()) g.set_introduction(row.at(3).as_string());
    if (!row.at(4).is_null()) g.set_faceurl(row.at(4).as_string());
    g.set_owneruserid(row.at(5).as_string());
    g.set_creatoruserid(row.at(6).as_string());
    ...
    if (!row.at(15).is_null()) g.set_notificationuserid(row.at(15).as_string());
    if (!row.at(16).is_null()) g.set_ex(row.at(16).as_string());
    return g;
}
...
sdkws::GroupMemberFullInfo parse_member_from_row(const boost::mysql::rows_view &rows, size_t idx)
{
    ...
    m.set_groupid(row.at(0).as_string());
    m.set_userid(row.at(1).as_string());
    m.set_rolelevel(row.at(2).as_int64());
    m.set_jointime(row.at(3).as_int64());
    if (!row.at(4).is_null()) m.set_nickname(row.at(4).as_string());
    if (!row.at(5).is_null()) m.set_faceurl(row.at(5).as_string());
    m.set_joinsource(row.at(6).as_int64());
    if (!row.at(7).is_null()) m.set_operatoruserid(row.at(7).as_string());
    if (!row.at(8).is_null()) m.set_inviteruserid(row.at(8).as_string());
    if (!row.at(9).is_null()) m.set_muteendtime(row.at(9).as_int64());
    if (!row.at(10).is_null()) m.set_ex(row.at(10).as_string());
    return m;
}
